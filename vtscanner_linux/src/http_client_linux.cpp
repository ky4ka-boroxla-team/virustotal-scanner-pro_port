...
void InitSSL() {
    if (!s_sslInit) {
        ...
        SSL_CTX_set_verify(s_ctx, SSL_VERIFY_PEER, nullptr);
...
SSL* ConnectSSL(int sock, const std::string& host, bool& ok) {
    ok = false;
...
bool FindHeaderValue(const std::string& headerBlock, const std::string& name, std::string& value) {
    std::string lowerBlock = headerBlock;
    std::string lowerName = name;
    std::transform(lowerBlock.begin(), lowerBlock.end(), lowerBlock.begin(), ::tolower);
    std::transform(lowerName.begin(), lowerName.end(), lowerName.begin(), ::tolower);
    size_t pos = lowerBlock.find(lowerName + ":");
    ...
    value = headerBlock.substr(valueStart, lineEnd - valueStart);
...
void ExtractResponse(const std::string& response, HttpResponse& result) {
    size_t pos = response.find("\r\n\r\n");
    if (pos == std::string::npos) return;
    result.status = GetStatusCode(response);
    std::string headerBlock = response.substr(0, pos);
    std::string rawBody = response.substr(pos + 4);

    std::string transferEncoding;
    if (FindHeaderValue(headerBlock, "transfer-encoding", transferEncoding)) {
        std::string lower = transferEncoding;
        std::transform(lower.begin(), lower.end(), lower.begin(), ::tolower);
        if (lower.find("chunked") != std::string::npos) {
            result.body = DecodeChunkedBody(rawBody);
            return;
        }
    }
    result.body = rawBody;
...
HttpResponse HttpClient::Get(const std::string& url,
    ...
    bool sslOk = false;
    SSL* ssl = ConnectSSL(sock, parts.host, sslOk);
    if (!sslOk) {
        if (ssl) SSL_free(ssl);
        close(sock);
    ...
    SSL_free(ssl);
    close(sock);
    ExtractResponse(response, result);
    return result;
}
