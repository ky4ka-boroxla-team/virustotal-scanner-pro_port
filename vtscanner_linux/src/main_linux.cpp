...
static Atom g_atomXdndAware = 0;
static Atom g_atomXdndEnter = 0;
static Atom g_atomXdndPosition = 0;
static Atom g_atomXdndStatus = 0;
static Atom g_atomXdndLeave = 0;
static Atom g_atomXdndDrop = 0;
static Atom g_atomXdndFinished = 0;
static Atom g_atomXdndSelection = 0;
static Atom g_atomXdndActionCopy = 0;
static Atom g_atomUriList = 0;
static Window g_xdndSource = None;
static Atom g_xdndTarget = 0;
static bool g_xdndPendingDrop = false;
...
static std::string UriToPath(const std::string& uri) {
    ...
    size_t slash = s.find('/');
    if (slash != std::string::npos && slash != 0) s = s.substr(slash);
    std::string out;
...
static void InitXdndAtoms() {
    g_atomXdndAware = XInternAtom(g_display, "XdndAware", False);
    g_atomXdndEnter = XInternAtom(g_display, "XdndEnter", False);
    g_atomXdndPosition = XInternAtom(g_display, "XdndPosition", False);
    g_atomXdndStatus = XInternAtom(g_display, "XdndStatus", False);
    g_atomXdndLeave = XInternAtom(g_display, "XdndLeave", False);
    g_atomXdndDrop = XInternAtom(g_display, "XdndDrop", False);
    g_atomXdndFinished = XInternAtom(g_display, "XdndFinished", False);
    g_atomXdndSelection = XInternAtom(g_display, "XdndSelection", False);
    g_atomXdndActionCopy = XInternAtom(g_display, "XdndActionCopy", False);
    g_atomUriList = XInternAtom(g_display, "text/uri-list", False);
}

static void EnableXdnd() {
    long version = 5;
    XChangeProperty(g_display, g_window, g_atomXdndAware, XA_ATOM, 32,
                     PropModeReplace, (unsigned char*)&version, 1);
}

static void HandleXdndEnter(const XClientMessageEvent& xev) {
    g_xdndSource = (Window)xev.data.l[0];
    bool moreTypes = (xev.data.l[1] & 1) != 0;
    g_xdndTarget = 0;
    if (!moreTypes) {
        for (int i = 2; i <= 4; ++i) {
            Atom a = (Atom)xev.data.l[i];
            if (a == g_atomUriList) { g_xdndTarget = a; break; }
        }
    } else {
        Atom actualType; int actualFormat;
        unsigned long nitems, bytesAfter;
        unsigned char* data = nullptr;
        XGetWindowProperty(g_display, g_xdndSource, XInternAtom(g_display, "XdndTypeList", False),
                            0, 1024, False, XA_ATOM, &actualType, &actualFormat, &nitems, &bytesAfter, &data);
        if (data) {
            Atom* atoms = (Atom*)data;
            for (unsigned long i = 0; i < nitems; ++i) {
                if (atoms[i] == g_atomUriList) { g_xdndTarget = atoms[i]; break; }
            }
            XFree(data);
        }
    }
...
static void HandleXdndPosition(const XClientMessageEvent& xev) {
    Window source = (Window)xev.data.l[0];
    XClientMessageEvent m = {};
    m.type = ClientMessage;
    m.display = g_display;
    m.window = source;
    m.message_type = g_atomXdndStatus;
    m.format = 32;
    m.data.l[0] = g_window;
    m.data.l[1] = g_xdndTarget != 0 ? 1 : 0;
    m.data.l[2] = 0;
    m.data.l[3] = 0;
    m.data.l[4] = g_xdndTarget != 0 ? g_atomXdndActionCopy : 0;
    XSendEvent(g_display, source, False, NoEventMask, (XEvent*)&m);
    XFlush(g_display);
}

static void HandleXdndLeave() {
    g_xdndSource = None;
    g_xdndTarget = 0;
}

static void HandleXdndDrop(const XClientMessageEvent& xev) {
    Window source = (Window)xev.data.l[0];
    if (g_xdndTarget == 0) {
        XClientMessageEvent m = {};
        m.type = ClientMessage;
        m.display = g_display;
        m.window = source;
        m.message_type = g_atomXdndFinished;
        m.format = 32;
        m.data.l[0] = g_window;
        m.data.l[1] = 0;
        m.data.l[2] = 0;
    ...
    g_xdndPendingDrop = true;
    XConvertSelection(g_display, g_atomXdndSelection, g_xdndTarget,
                       g_atomXdndSelection, g_window, xev.data.l[2]);
}

static void HandleXdndSelectionNotify(const XSelectionEvent& sel) {
    if (!g_xdndPendingDrop) return;
    g_xdndPendingDrop = false;
    if (sel.property != None) {
        ...
        if (data) {
            std::string uriList((char*)data, nitems);
            XFree(data);
            size_t end = uriList.find_first_of("\r\n");
            std::string firstUri = end == std::string::npos ? uriList : uriList.substr(0, end);
            if (!firstUri.empty() && g_app) {
                g_app->OnFileDropped(UriToPath(firstUri));
            }
        }
    }
    XClientMessageEvent m = {};
    m.type = ClientMessage;
    m.display = g_display;
    m.window = g_xdndSource;
    m.message_type = g_atomXdndFinished;
    m.format = 32;
    m.data.l[0] = g_window;
    m.data.l[1] = 1;
    m.data.l[2] = g_atomXdndActionCopy;
    XSendEvent(g_display, g_xdndSource, False, NoEventMask, (XEvent*)&m);
    XFlush(g_display);
    g_xdndSource = None;
    g_xdndTarget = 0;
}
    ...
    while (!done) {
        ...
        while (XPending(g_display)) {
            ...
            if (event.type == ClientMessage) {
                Atom mt = event.xclient.message_type;
                if (mt == g_atomXdndEnter) HandleXdndEnter(event.xclient);
                else if (mt == g_atomXdndPosition) HandleXdndPosition(event.xclient);
                else if (mt == g_atomXdndLeave) HandleXdndLeave();
            ...
            if (event.type == SelectionNotify && event.xselection.selection == g_atomXdndSelection) {
